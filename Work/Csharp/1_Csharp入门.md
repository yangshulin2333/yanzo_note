...
###### 1. **逻辑与 (`&&`)**
   - **用法**: `expr1 && expr2`
   - **行为**:
     - 如果`expr1`为`false`，则`expr2`不会被执行或检查，结果为`false`。
     - 如果`expr1`为`true`，则`expr2`会被执行，结果取决于`expr2`。
   - **用途**: 在确保多个条件必须同时为真时使用。

###### 2. **逻辑或 (`||`)**
   - **用法**: `expr1 || expr2`
   - **行为**:
     - 如果`expr1`为`true`，则`expr2`不会被执行或检查，结果为`true`。
     - 如果`expr1`为`false`，则`expr2`会被执行，结果取决于`expr2`。
   - **用途**: 在只需有一个条件为真即可的情况下使用。

###### 3. **逻辑非 (`!`)**
   - **用法**: `!expr`
   - **行为**:
     - 取反操作，如果`expr`为`true`，结果为`false`；反之亦然。
   - **用途**: 用于反转布尔值。

###### 短路逻辑

- **效率**: 短路特性可以避免不必要的计算，特别是在操作代价较高或可能导致错误的情况下。
- **常见用法**: 在链式条件判断中，通过短路避免空引用异常和其他潜在问题。

###### 示例

```csharp
bool IsValid(int number, List<int> list)
{
    return number >= 0 && number < list.Count && list[number] > 0;
}
```

在这个例子中，`list[number] > 0`只有在`number`在有效范围内时才会被检查，确保了安全的访问操作。
...
##### 控制台窗口设置
![](assets/1_Csharp入门/file-20260106173356826.png)
保存后，重新启动
